...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case GLFW_KEY_UP:
			...
			scn->change_direction(Eigen::Vector3d(0, 0.01, 0));
			break;
		case GLFW_KEY_DOWN:
			//rndr->TranslateCamera(Eigen::Vector3f(0, -0.01f,0));
			scn->change_direction(Eigen::Vector3d(0, -0.01, 0));
		...
		case GLFW_KEY_LEFT:
			...
			scn->change_direction(Eigen::Vector3d(-0.01, 0, 0));
			break;
		case GLFW_KEY_RIGHT:
			//rndr->TranslateCamera(Eigen::Vector3f(0.01f, 0, 0));
			scn->change_direction(Eigen::Vector3d(0.01, 0, 0));
			break;
		case GLFW_KEY_SPACE:
			scn->SimplifyMesh(std::ceil(scn->getQsize(scn->data().id) * 0.05));
			//scn->SimplifyMesh(1);
			break;
		case GLFW_KEY_EQUAL:
			scn->print_data_structure();
			break;
		case GLFW_KEY_MINUS:
			scn->change_direction(Eigen::Vector3d(0, 0, 0));
			break;
		//case ' ':
